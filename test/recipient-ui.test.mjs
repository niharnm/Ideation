...
test("Fieldline restaurant view starts with a locked scope and no rendered allergy detail", () => {
  ...
  assert.match(html, /No credential-backed fact has been shared\./);
...
test("approved restaurant scope is minimized to the customer-approved field", () => {
  ...
  assert.match(app, /const fields = workspace\.recipientRequest\.scopedFields/);
  assert.match(app, /One verified fact/);
  assert.match(app, /\$\{fields\.length\} verified facts/);
...
test("revocation removes scope detail and locks later restaurant actions", () => {
  ...
  assert.match(app, /No credential-backed fact has been shared/);
  ...
  assert.match(app, /recordRecipientDecision/);
...
test("business UI contains merchant queue, active scope, kitchen actions, and status history", () => {
  ...
  assert.match(html, /id="scope-card"/);
  assert.match(app, /Verified order fact active/);
...
test("restaurant does not ask AI to make allergy-safety recommendations", () => {
  assert.doesNotMatch(html, /AI Chef Smart Suggestion/);
  assert.doesNotMatch(app, /smart-prep/);
});
